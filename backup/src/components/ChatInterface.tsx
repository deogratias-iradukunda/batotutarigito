...
export function ChatInterface() {
  const [messages, setMessages] = useState<Message[]>(() => {
    ...
    return [
      {
        role: Role.MODEL,
        content: "Hello! I'm Aether. How can I help you today?",
        timestamp: Date.now(),
      },
    ];
  });
  ...
  useEffect(() => {
    localStorage.setItem("aether-chat-history", JSON.stringify(messages));
    scrollToBottom();
  }, [messages]);
  ...
  const startNewChat = () => {
    ...
    localStorage.setItem("aether-chat-history", JSON.stringify(defaultMessages));
    setError(null);
  };
    ...
    <div className="flex flex-col h-screen max-w-4xl mx-auto bg-white">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-gray-900 flex items-center gap-2">
              Aether AI <Sparkles className="w-4 h-4 text-amber-500 fill-amber-500" />
            </h1>
      ...
      <div className="p-4 md:p-6 bg-white">
        ...
        <p className="mt-3 text-center text-[10px] text-gray-400">
          Aether may produce inaccurate information about people, places, or facts.
        </p>
      </div>
    </div>
  );
